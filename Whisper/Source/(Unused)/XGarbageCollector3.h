 ...
 *
 *		$Log: XGarbageCollector3.h,v $
 *		Revision 1.3  2001/03/03 11:15:52  jesjones
 *		Instead of specializing selected algorithms for POD types, we use the new MSL mechanism to declare POD types.
 *		
 *		Revision 1.2  2000/11/09 12:37:31  jesjones
...
public:
			...
			void 		AssignInterface(XBoss* boss, const XUnknown* imp);
			void 		RemoveInterface(XBoss* boss, const XUnknown* imp);
						// A few interface implementations store containers of XInterfacePtr's.
						// In order to garbage collect in a timely manner these classes need
						// to tell the collector which interfaces they're pointing to.
	
			void 		InterfacePtrChanged(const ZBaseInterfacePtr* ptr, const XUnknown* oldImp);
						// Called by ZBaseInterfacePtr as the face pointer changes.
...
public:
	...
	struct SBoss {
		XBoss*									boss;		// there will be one entry with a nil boss for stack based XInterfacePtr's
		mutable uint32							generation;
		mutable std::multiset<const XUnknown*>	ptrs;		// interfaces the boss references (usually via XInterfacePtr's as member data of one of the boss's interfaces)
